...
package com.xpread.swipelistview;
...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    private List<Boolean> right2Open = new ArrayList<Boolean>();
    ...
    protected void openAnimate(int position) {
        final View child = swipeListView.getChildAt(
                position - swipeListView.getFirstVisiblePosition()).findViewById(swipeFrontView);
    ...
    protected void toggleAnimate(int position) {
        if (swipeListView != null) {
            int firstVisibleChildPosition = swipeListView.getFirstVisiblePosition();
            final View childContainer = swipeListView.getChildAt(position
                    - firstVisibleChildPosition);
            if (childContainer != null) {
                final View child = childContainer.findViewById(swipeFrontView);

                if (child != null) {
                    toggleAnimate(child, position);
                }
            }
        }
    }
    ...
    private void openAnimate(View view, int position) {
        if (!opened.get(position)) {
            generateRevealAnimate(view, true, right2Open.get(position), position);
        }
    }
    ...
    private void toggleAnimate(View view, int position) {
        // 切换动画
        generateRevealAnimate(view, true, right2Open.get(position) && !opened.get(position),
                position);
    }
     ...
     *            position
     * @param swapRight If swap is true, this parameter tells if move is to the
     *            right or left
     * @param position Position of list
     */
    private void generateAnimate(final View view, final boolean swap, final boolean swapRight,
            final int position) {
        if (SwipeListView.DEBUG) {
            Log.d(SwipeListView.TAG, "swap: " + swap + " - swapRight: " + swapRight
                    + " - position: " + position);
        }
     ...
     *            position
     * @param swapRight If swap is true, this parameter tells if movement is
        ...
        animate(view).translationX(moveTo).setDuration(animationTime)
                .setListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (swap && swipeListView.getAdapter().getCount() > 0) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity
                    ...
                    if (SwipeListView.DEBUG) {
                        Log.d(SwipeListView.TAG, "swapRight: " + swapRight + " - swipingRight: "
                                + swipingRight);
                    }
                    if (swapRight != swipingRight && swipeActionLeft != swipeActionRight) {
                        ...
                        if (!opened.get(downPosition)
                                && swipeMode == SwipeListView.SWIPE_MODE_RIGHT && !swapRight) {
                            swap = false;
                        } else if (!opened.get(downPosition)
                                && swipeMode == SwipeListView.SWIPE_MODE_LEFT && swapRight) {
                            swap = false;
                        } else {
                            swap = true;
                        }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE
                    ...
                    if (SwipeListView.DEBUG) {
                        Log.d(SwipeListView.TAG, "deltaX: " + deltaX + " - swipingRight: "
                                + swipingRight);
                    }
                    if (opened.get(downPosition)) {
